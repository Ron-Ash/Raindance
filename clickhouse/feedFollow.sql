...
AS SELECT _key AS `user`, `follows`, now() AS `event_time`, 1 AS `sign` FROM socialNetwork_followSystem_kafka;
...
AS SELECT _key AS `user`, `follows`, now() AS `event_time`, -1 AS `sign` FROM socialNetwork_unfollowSystem_kafka;
